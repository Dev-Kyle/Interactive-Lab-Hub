...
if encoder_connected:
    try:
        ...
        # --- NEW: Set up the encoder's button pin ---
...
def dino_game_loop():
    ...
    while True:
        # --- NEW: Check for encoder button press to exit to menu ---
        ...
        if game_over:
            if (joystick_connected and myJoystick.button == 0):
                return "MENU" # Let button press also go to menu
        else:
            if joystick_connected:
...
class MazePlayer(pygame.sprite.Sprite):
    ...
    def move(self, dx, dy, walls):
        ...
        # Check bounds and if there's a wall in the way
        if 0 <= new_x < MAZE_COLS and 0 <= new_y < MAZE_ROWS:
            # Wall check logic: L,R,U,D
...
def generate_maze():
    ...
    while stack:
        ...
        if x > 0 and (x-1, y) not in visited: neighbors.append((x-1, y, 0, 1)) # Left, my wall, neighbor wall
        if x < MAZE_COLS-1 and (x+1, y) not in visited: neighbors.append((x+1, y, 1, 0)) # Right
        if y > 0 and (x, y-1) not in visited: neighbors.append((x, y-1, 2, 3)) # Up
        if y < MAZE_ROWS-1 and (x, y+1) not in visited: neighbors.append((x, y+1, 3, 2)) # Down
    ...
    while True:
        ...
        for r in range(MAZE_ROWS):
            for c in range(MAZE_COLS):
                ...
                if walls[r][c][0]: pygame.draw.line(screen, BLACK, (x, y), (x, y + CELL_SIZE)) # Left
                if walls[r][c][1]: pygame.draw.line(screen, BLACK, (x + CELL_SIZE, y), (x + CELL_SIZE, y + CELL_SIZE)) # Right
                if walls[r][c][2]: pygame.draw.line(screen, BLACK, (x, y), (x + CELL_SIZE, y)) # Up
                if walls[r][c][3]: pygame.draw.line(screen, BLACK, (x, y + CELL_SIZE), (x + CELL_SIZE, y + CELL_SIZE)) # Down
